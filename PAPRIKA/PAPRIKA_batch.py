# PAPRIKA with constant lambda simulation code

import numpy as np


class PAPRIKA_proc_batch:
    def __init__(self, alpha0, numhyp, lbd, gamma_vec_exponent,eps, sensitivity, cutoff,shift):
        ...
        self.w0 = (1 - 2*lbd) * self.alpha0/2 # initial wealth, has to satisfy w0 < (1 - lbd)*alpha0
    ...
    def run_fdr(self, pvec):
        ...
        for k in range(0, numhyp):
            ...
            candidates[k + 1] = (pvec[k] < 2*self.lbd)
            ...
            # Update wealth
            wealth = self.wealth_vec[k] - (1 - candidates[k + 1]) * this_alpha + rej[k + 1] * (1 - 2*self.lbd) * (
            ...
            if len(last_rej) > 0:
                ...
                if len(last_rej) >= 2:
                    ...
                    sum_gam = 0
                next_alpha = min(self.lbd, zero_gam * self.w0 + ((1-2*self.lbd)*self.alpha0 - self.w0) * first_gam + (1-2*self.lbd)*self.alpha0 * sum_gam)
            else:
